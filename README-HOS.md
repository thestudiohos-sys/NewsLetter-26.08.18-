...
## 상품 입력 UI 미리보기 (TASK-08 WIP)

JSON 파일을 직접 편집하지 않고 브라우저에서 입력 구조를 확인하려면 다음 명령을 실행합니다.

```powershell
npm run playground:smartstore-ui
```

브라우저에서 `http://127.0.0.1:4173`을 열면 스토어와 상품 정보를 입력하고 SmartStoreInput 형식의 JSON Preview를 확인할 수 있습니다. 상품은 1~5개, 상품 특징은 각 상품당 최소 1개를 유지합니다.

현재 화면은 입력 UI 골격입니다. Preview를 파일로 저장하거나 뉴스레터를 생성하는 기능, 상품 URL 자동수집, 이미지 분석 기능은 아직 연결되지 않았습니다.
