...
describe("smartStoreInputSchema", () => {
  ...
  test("Hero와 상품 이미지 경로를 선택적으로 보존한다", () => {
    ...
    const withImages = {
      ...
      products: [
        {
          ...input.products[0]!,
          mainImage: "playground/assets/products/item-main.jpg",
          secondaryImage: "playground/assets/products/item-sub.jpg",
        },
      ],
    };

    expect(parseSmartStoreInput(withImages)).toEqual(withImages);
  });

  test("이미지가 없는 입력과 secondaryImage가 없는 입력을 허용한다", () => {
    const withoutImages = createInput(1);
    const withMainImageOnly = {
      ...withoutImages,
      products: [
        {
          ...withoutImages.products[0]!,
          mainImage: "playground/assets/products/item-main.jpg",
        },
      ],
    };

    expect(parseSmartStoreInput(withoutImages)).toEqual(withoutImages);
    expect(parseSmartStoreInput(withMainImageOnly)).toEqual(withMainImageOnly);
  });
  ...
  test("빈 이미지 경로를 거부한다", () => {
    ...
    expect(
      smartStoreInputSchema.safeParse({ ...input, heroImage: "  " }).success,
    ).toBe(false);
    expect(
      smartStoreInputSchema.safeParse({
        ...input,
        products: [{ ...input.products[0]!, mainImage: "" }],
      }).success,
    ).toBe(false);
  });
